...
!(function (a) {
    ...
    function b(a, b) {
        ...
        return (
            ...
            Promise.resolve(a)
                ...
                .then(c)
                .then(function (c) {
                    return k(c, b.width || a.scrollWidth, b.height || a.scrollHeight);
                })
        );
    }
    function c(a, b) {
        return g(a, b || {}).then(function (b) {
            return b
                .getContext("2d")
                .getImageData(0, 0, a.scrollWidth, a.scrollHeight).data;
        });
    }
    function d(a, b) {
    ...
    function g(a, c) {
        function d(a) {
            ...
            return (
                (b.width = c.width || a.scrollWidth),
                (b.height = c.height || a.scrollHeight),
                b
            );
        }
        return b(a, c)
...
const buildPng = (el, filename) => {
    domtoimage.toPng(el, { quality: 1 }).then((dataUrl) => {
...
const buildSvg = (el, filename) => {
    domtoimage.toSvg(el, { quality: 1 }).then((dataUrl) => {
        var link = document.createElement("a");
...
$(() => {
    ...
    downloadPng.on("click", () => {
        const filename = "coracard";
        buildPng(node, filename);
    });

    const downloadSvg = $(".download-svg");
    downloadSvg.on("click", () => {
        const filename = "coracard";
        buildSvg(node, filename);
    });
